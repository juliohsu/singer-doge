...
class DogeMultiModalProjector(nn.Module):
    def __init__(self, config: DogeConfig):
        super().__init__()
        self.hidden_dim = config.hidden_size
        self.proj = nn.Linear(self.hidden_dim, self.hidden_dim, bias=config.hidden_bias)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ensure that the dimensions of text_states and audio_states are both [batch_size, seq_len, hidden_dim]
        assert text_states.shape == audio_states.shape

        # concatenate on the seq_len dimension
        hidden_states = torch.cat([text_states, audio_states], dim=1)

        hidden_states = self.proj(hidden_states)
        return hidden_states


class DogeForCausalALM(DogePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: DogeConfig):
        ...
        self.model = DogeModel(config)
        self.vocab_size = config.vocab_size
        self.alm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
